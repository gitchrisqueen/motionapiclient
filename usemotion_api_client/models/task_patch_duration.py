...
class TaskPatchDuration(BaseModel):
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    one_of_schemas: List[str] = Field(TASKPATCHDURATION_ONE_OF_SCHEMAS,
                                      const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TaskPatchDuration.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in TaskPatchDuration with oneOf schemas: float, str. Details: "
                + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in TaskPatchDuration with oneOf schemas: float, str. Details: "
                + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> TaskPatchDuration:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> TaskPatchDuration:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into TaskPatchDuration with oneOf schemas: float, str. Details: "
                + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into TaskPatchDuration with oneOf schemas: float, str. Details: "
                + ", ".join(error_messages))
        else:
            return instance
